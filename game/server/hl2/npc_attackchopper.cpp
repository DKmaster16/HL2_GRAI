...
bool CNPC_AttackHelicopter::DoGunCharging()
{
	...
	{
		m_nRemainingBursts = sk_helicopter_burstcount_long.GetInt();
		m_flIdleTimeDelay = 0.0f;
		nHitFactor = 2;
	}
	break;
	...
	if (!GetEnemyVehicle())
	{
		m_nMaxBurstHits = !IsDeadlyShooting() ? random->RandomInt(sk_helicopter_burst_min_hitcount.GetFloat(), sk_helicopter_burst_min_hitcount.GetFloat()) : 200;
