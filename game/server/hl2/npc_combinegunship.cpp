...
ConVar sk_gunship_belly_blast("sk_gunship_belly_blast", "150" );
...
void CNPC_CombineGunship::Spawn( void )
{
	...
	m_flBurstDelay = sk_gunship_burst_delay.GetFloat();
	...
	m_bCanGroundAttack = true;	// ( m_spawnflags & SF_GUNSHIP_NO_GROUND_ATTACK ) ? false : true;
...
bool CNPC_CombineGunship::CheckGroundAttack( void )
{
	...
	Vector vecToTarAget = (vecTarget - GetAbsOrigin());
	float flDistance = vecToTarAget.Length();
	...
	if (flDistance < 256 )	//&& flVelocityDiff < 200
	{
		return true;
	}
	*/
...
void CNPC_CombineGunship::DoBellyBlastDamage( trace_t &tr, Vector vMins, Vector vMaxs )
{
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		float damage = sk_gunship_belly_blast.GetFloat();
#if 0
		if ( pEntity->IsPlayer() )
		{
