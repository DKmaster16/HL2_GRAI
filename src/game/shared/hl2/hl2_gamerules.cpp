...
ConVar	sk_ar2_rampup_step			("sk_ar2_rampup_step", "1", FCVAR_REPLICATED);
ConVar	sk_ar2_rampup_max			("sk_ar2_rampup_max", "18", FCVAR_REPLICATED);
...
ConVar	sk_plr_smg1_grenade_speed ( "sk_plr_smg1_grenade_speed", "1000", FCVAR_REPLICATED );
ConVar	sk_npc_smg1_grenade_speed ( "sk_npc_smg1_grenade_speed", "600", FCVAR_REPLICATED );
...
ConVar	sk_357_fire_rate		( "sk_357_fire_rate", "0.75", FCVAR_REPLICATED );
...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	// Output : How much hurt to put on dude what done got shot (pVictim).
	//-----------------------------------------------------------------------------
	float CHalfLife2::GetAmmoVelocity( int nAmmoType )
	{
		float flVelocity = 0;
		CAmmoDef *pAmmoDef = GetAmmoDef();

		flVelocity = pAmmoDef->Velocity(nAmmoType);
...
#define BULLET_SPEED(ftpersec) (12*ftpersec*TICK_INTERVAL)	// inches per tick
#define BULLET_MASS(grains) (BULLET_MASS_GRAINS_TO_KG(grains)*100)	// centigrams
#define BULLET_DIAMETER(inches) (inches/2)	// inch/2
CAmmoDef *GetAmmoDef()
{
	static CAmmoDef def;
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("AR2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_ar2",			"sk_npc_dmg_ar2",			"sk_max_ar2",			BULLET_SPEED(667),	BULLET_MASS(520), BULLET_DIAMETER(3), BULLET_IMPULSE(520, 667), AMMO_DARK_ENERGY);
		...
		def.AddAmmoType("357",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_357",			"sk_npc_dmg_357",			"sk_max_357",			BULLET_SPEED(1550),	BULLET_MASS(125), BULLET_DIAMETER(.357), BULLET_IMPULSE(125, 1550), 0 );	
		...
		def.AddAmmoType("SniperRound",		DMG_BULLET | DMG_SNIPER,	TRACER_NONE,			"sk_plr_dmg_sniper_round",	"sk_npc_dmg_sniper_round",	"sk_max_sniper_round",	BULLET_SPEED(667), BULLET_MASS(5200), BULLET_DIAMETER(1), BULLET_IMPULSE(5200, 667), AMMO_DARK_ENERGY );
		def.AddAmmoType("SniperPenetratedRound", DMG_BULLET | DMG_SNIPER, TRACER_NONE,			"sk_dmg_sniper_penetrate_plr", "sk_dmg_sniper_penetrate_npc", "sk_max_sniper_round", BULLET_SPEED(667), BULLET_MASS(1040), BULLET_DIAMETER(1), BULLET_IMPULSE(1040, 667), AMMO_DARK_ENERGY );
		...
		def.AddAmmoType("AirboatGun",		DMG_AIRBOAT,				TRACER_LINE,			"sk_plr_dmg_airboat",		"sk_npc_dmg_airboat",		NULL,			BULLET_SPEED(667), BULLET_MASS(150),	BULLET_DIAMETER(6), BULLET_IMPULSE(2080, 667),  AMMO_DARK_ENERGY );
		def.AddAmmoType("StriderMinigun",	DMG_BULLET,					TRACER_LINE,			"sk_npc_dmg_strider_to_plr", "sk_npc_dmg_strider", "sk_npc_dmg_strider", BULLET_SPEED(667), BULLET_MASS(5200),	BULLET_DIAMETER(15), 1.0 * 667 * 12, AMMO_FORCE_DROP_IF_CARRIED | AMMO_TRACE_HULL | AMMO_DARK_ENERGY ); // hit like a 1.0kg weight at 750 ft/s
		...
		def.AddAmmoType("HelicopterGun",	DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_npc_dmg_helicopter_to_plr", "sk_npc_dmg_helicopter",	"sk_max_smg1",	BULLET_SPEED(667), BULLET_MASS(150),	BULLET_DIAMETER(1), BULLET_IMPULSE(2080, 667), AMMO_FORCE_DROP_IF_CARRIED | AMMO_TRACE_HULL | AMMO_INTERPRET_PLRDAMAGE_AS_DAMAGE_TO_PLAYER | AMMO_DARK_ENERGY);
		def.AddAmmoType("AR2AltFire",		DMG_DISSOLVE,				TRACER_NONE,			0, 0, "sk_max_ar2_altfire", 0, 0, 0, 0, 0 );
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_grenade",		"sk_npc_dmg_grenade",		"sk_max_grenade",	0, 0, 0, 0, 0 );
#ifdef HL2_EPISODIC
		def.AddAmmoType("Hopwire",			DMG_BLAST,					TRACER_NONE,			"sk_plr_dmg_grenade",		"sk_npc_dmg_grenade",		"sk_max_hopwire",	0, 0, 0, 0, 0 );
		def.AddAmmoType("CombineHeavyCannon",	DMG_BULLET,				TRACER_LINE,			"sk_heavy_cannon_damage_to_plr", "sk_heavy_cannon_damage", NULL, BULLET_SPEED(3000), BULLET_MASS(5200), BULLET_DIAMETER(2), 10 * 750 * 12, AMMO_FORCE_DROP_IF_CARRIED | AMMO_DARK_ENERGY | AMMO_INTERPRET_PLRDAMAGE_AS_DAMAGE_TO_PLAYER ); // hit like a 10 kg weight at 750 ft/s
		def.AddAmmoType("ammo_proto1",			DMG_BULLET,				TRACER_LINE,		0,	0, 0, 0, 0, 10, 0, 0 );
#endif // HL2_EPISODIC
	}
