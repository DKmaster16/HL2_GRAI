...
ConVar	ai_use_think_optimizations( "ai_use_think_optimizations", "0" );	// was set to one, we now have better CPUs so disable optimizations 
...
ConVar	sk_npc_head( "sk_npc_head","4" );		// was 2
ConVar	sk_npc_chest( "sk_npc_chest","2" );		// was 1
...
//		}
	...
	if (iSoundTypeNoContext & SOUND_PLAYER)
	{
		return SOUND_PRIORITY_HIGH;
	}

	if (iSoundTypeNoContext & SOUND_BULLET_IMPACT)
	{
		return SOUND_PRIORITY_VERY_HIGH;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
ConVar sk_always_flinch("sk_always_flinch", "0");
...
void CAI_BaseNPC::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	if (gpGlobals->curtime - GetEnemyLastTimeSeen() > 8)
	{
		...
		if (!HasCondition(COND_SEE_ENEMY) && HasCondition(COND_ENEMY_UNREACHABLE))
		{
			...
			if ( flDist >= 512 && !FVisible(flEnemyLKP))
			{
				MarkEnemyAsEluded();
			}
		}
...
Vector CAI_BaseNPC::GetActualShootPosition(const Vector &shootOrigin)
{
	...
	if (ai_lead_targets.GetBool() == 1)
	{
		...
		if ( pWeapon )
		{
			bulletVelocity = GetAmmoDef()->Velocity(pWeapon->GetPrimaryAmmoType()) * 100;
		}
		else
		{
			bulletVelocity = 8000; // 8000 is the velocity of energy bullets
		}

		// Get bullet time to target
		targetDist = (vecTargetPosition - GetAbsOrigin()).Length();
		targetTime = targetDist / bulletVelocity;
