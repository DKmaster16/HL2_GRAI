...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
#define PC_LARGER_BURST_RANGE	(12.0f * 21.0f) // If an enemy is this close, player companions fire larger continuous bursts.
...
//-----------------------------------------------------------------------------
#define CITIZEN_HEADSHOT_FREQUENCY	2	// one in this many shots at a zombie will be aimed at the zombie's head
#define VITAL_ALLY_HEADSHOT_FREQUENCY 2	// one in this many shots at any target will be aimed at the enemy's head
Vector CNPC_PlayerCompanion::GetActualShootPosition(const Vector &shootOrigin)
{
	if (Classify() == CLASS_PLAYER_ALLY_VITAL && random->RandomInt(1, VITAL_ALLY_HEADSHOT_FREQUENCY) == 1)
	{
		return GetEnemy()->HeadTarget(shootOrigin);
	}
	else if (GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE && random->RandomInt(1, CITIZEN_HEADSHOT_FREQUENCY) == 1)
	{
		return GetEnemy()->HeadTarget(shootOrigin);
	}
...
float CNPC_PlayerCompanion::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_HEAD:
...
WeaponProficiency_t CNPC_PlayerCompanion::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
	...
	{
		return WEAPON_PROFICIENCY_POOR;		// 5/10 AR2
	}
		return WEAPON_PROFICIENCY_GOOD;		// 8.33/11.11 SMG1
...
/*
	...
	{
		return WEAPON_PROFICIENCY_AVERAGE;	// AR2 = 5, SMG1 = 8.25
	}
	else if (chance > 90)	// 10% chance to be a good shot
	{
		return WEAPON_PROFICIENCY_VERY_GOOD; // AR2 = 3, SMG1 = 5
	}
	else if (chance > 80 && chance < 90)	// 10% chance to be terrible shot
	{
		return WEAPON_PROFICIENCY_POOR; // AR2 = 9, SMG1 = 16.5
	}
	else					// 30% chance to be above average
	{
