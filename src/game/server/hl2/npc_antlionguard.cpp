...
public:
	...
	virtual float GetAutoAimRadius() { return 60.0f; }
...
impactdamagetable_t gAntlionGuardImpactDamageTable =
{
	...
	15,		// anything less than 15kg is "small"
	5,		// never take more than 5 pt of damage from anything under 15kg
...
int CNPC_AntlionGuard::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	// Cap damage taken by dissolving (otherwise we can get killed by combine elite)
	if ((dInfo.GetDamageType() & DMG_DISSOLVE ))
	{
		dInfo.SetDamage(50);
	}
...
void CNPC_AntlionGuard::PrescheduleThink( void )
{
	...
	for ( int i = 0; i < 4; i++ )	// old: 3
	{
		CSoundEnt::InsertSound( SOUND_DANGER, WorldSpaceCenter() + ( BodyDirection3D() * 128 * (i+1) ), 96, 0.1f, this );		
	}
...
bool CNPC_AntlionGuard::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_CRUSH )
	{
		...
		if ( ( pPhysObject != NULL )  && ( pPhysObject->GetGameFlags() & FVPHYSICS_WAS_THROWN )) //removed, otherwise instant death when hit by elite's energy ball
		{
