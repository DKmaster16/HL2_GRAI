...
void CHLMachineGun::DoMachineGunKick( CBasePlayer *pPlayer, float dampEasy, float maxVerticleKickAngle, float fireDurationTime, float slideLimitTime )
{
	...
	if ( g_pGameRules->IsSkillLevel( SKILL_STORY ) || g_pGameRules->IsSkillLevel( SKILL_EASY ) || g_pGameRules->IsSkillLevel( SKILL_MEDIUM ) )
	{
		for ( int i = 0; i < 3; i++ )
		{
...
void CHLSelectFireMachineGun::ItemPostFrame(void)
{
	...
	if (!m_bInReload && m_iClip1 > 0 && (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime) && m_iFireMode == FIREMODE_SEMI)
	{
		...
		if (m_nShotsFired == 0)
		{
			...
			Vector vecAiming = pOwner->GetAutoaimVector(0.8);
...
*/
	...
	if ((pOwner->m_nButtons & IN_ATTACK) == false)
	{
		m_nShotsFired-= 2;	// was m_nShotsFired = 0
	}
