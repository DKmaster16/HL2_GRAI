...
#define RECENT_DAMAGE_INTERVAL		1.0f	// was 3.0f
#define RECENT_DAMAGE_THRESHOLD		0.1f	// was 0.2f
...
ConVar	sk_metropolice_model_scale_min("sk_metropolice_model_scale_min", "1.00");
ConVar	sk_metropolice_model_scale_max("sk_metropolice_model_scale_max", "1.00");
ConVar	sk_metrocop_speed_scale("sk_metrocop_speed_scale", "1.0");
ConVar	sk_metrocop_stunstick_speed_scale("sk_metrocop_stunstick_speed_scale", "1.08");
ConVar	sk_metropolice_stitch_distance("sk_metropolice_stitch_distance", "1000");

ConVar	sk_metrocop_max_additional_ammo_shots("sk_metrocop_max_additional_ammo_shots", "4");
ConVar	sk_metrocop_vest("sk_metrocop_vest", "0.5");
...
void CNPC_MetroPolice::Spawn(void)
{
	...
	float mdl_scale = random->RandomFloat(sk_metropolice_model_scale_min.GetFloat(), sk_metropolice_model_scale_max.GetFloat());
	SetModelScale(mdl_scale);
	if (m_bSimpleCops)
	{
		m_iHealth = sk_metropolice_simple_health.GetFloat() * pow(mdl_scale, 4);
	}
	else
	{
...
const float MAX_MIN_PISTOL_REST_INTERVAL = 1.25;	// The maximum time in a minimum rest time (He's very far)
...
void CNPC_MetroPolice::OnUpdateShotRegulator()
{
	...
	if (Weapon_OwnsThisType("weapon_pistol"))
	{
		if (m_nBurstMode == BURST_NOT_ACTIVE)
		{
			...
			else
			{
				...
				GetShotRegulator()->SetRestInterval(0.4, 1.4);
			}
		}
	...
	else if (Weapon_OwnsThisType("weapon_smg1"))
	{
		...
		{
			if (GetAbsOrigin().DistTo(GetEnemy()->GetAbsOrigin()) <= METROCOP_LARGER_BURST_RANGE)
			{
				// Longer burst
				int longBurst = random->RandomInt(15, 25);
...
void CNPC_MetroPolice::PainSound(const CTakeDamageInfo &info)
{
	...
	if (healthRatio > 0.0f)
	{
		...
		if (!HasMemory(bits_MEMORY_PAIN_HEAVY_SOUND) && (healthRatio < 0.4f))
		{
...
Activity CNPC_MetroPolice::NPC_TranslateActivity(Activity newActivity)
{
	...
	// !!!HACK - Metrocops don't have the required animations for shotguns, 
	// so trick them into using the SMG1 counterparts for now (sjb)
	...
	if (newActivity == ACT_RELOAD_LOW)
		return ACT_RELOAD_SMG1_LOW;
	if (newActivity == ACT_COVER_LOW)
...
void CNPC_MetroPolice::Event_Killed(const CTakeDamageInfo &info)
{
	...
	pHL2GameRules->NPC_TakenAdditionalShots(0);
...
int CNPC_MetroPolice::SelectScheduleNoDirectEnemy()
{
	...
	if (OccupyStrategySlot(SQUAD_SLOT_POLICE_HARASS))
	{
...
int CNPC_MetroPolice::SelectCombatSchedule()
{
	...
	*/
	...
	if (HasCondition(COND_WEAPON_SIGHT_OCCLUDED) && !HasBaton())
	{
		...
		if (pBlocker &&  OccupyStrategySlotRange(SQUAD_SLOT_POLICE_ATTACK_OCCLUDER1, SQUAD_SLOT_POLICE_ATTACK_OCCLUDER2))	// pBlocker->GetHealth() > 0 && got rid of it, OPEN FIRE ANYWAY
		{
...
{
	// Metropolice considers any enemy damage heavy damage
...
void CNPC_MetroPolice::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	...
	if (pPlayer != NULL)
	{
		...
		if (info.GetDamageType() & DMG_BULLET)
		{
			...
			{
				if (pHL2GameRules->NPC_ShouldDrainPistolAmmo(pPlayer) && m_flAdditionalShots < sk_metrocop_max_additional_ammo_shots.GetInt())
				{
					// Take only one additioanl headshot.
					if ( ptr->hitgroup == HITGROUP_HEAD && subInfo.GetDamage() * sk_npc_head.GetFloat() >= (float)GetHealth() )
					{
						subInfo.SetDamage(1);
						pHL2GameRules->NPC_TakenAdditionalShots(1);
						m_flAdditionalShots = sk_metrocop_max_additional_ammo_shots.GetInt();
					}
			...
			{
				if (pHL2GameRules->NPC_ShouldDrainAR2Ammo(pPlayer) && m_flAdditionalShots < sk_metrocop_max_additional_ammo_shots.GetInt())
				{
					// Take only one additioanl headshot.
					if (ptr->hitgroup == HITGROUP_HEAD && subInfo.GetDamage() * sk_npc_head.GetFloat() >= (float)GetHealth())
					{
			...
			{
				if (pHL2GameRules->NPC_ShouldDrainSMGAmmo(pPlayer) && m_flAdditionalShots < sk_metrocop_max_additional_ammo_shots.GetInt())
				{
					// Take only one additioanl headshot.
					if (ptr->hitgroup == HITGROUP_HEAD && subInfo.GetDamage() * sk_npc_head.GetFloat() >= (float)GetHealth())
					{
			...
			{
				if (pHL2GameRules->NPC_ShouldDrain357Ammo(pPlayer) && m_flAdditionalShots < sk_metrocop_max_additional_ammo_shots.GetInt()
					&& ptr->hitgroup != HITGROUP_HEAD && subInfo.GetDamage() >= (float)GetHealth())
				{
					subInfo.ScaleDamage(0.5);
					pHL2GameRules->NPC_TakenAdditionalShots(1);
					m_flAdditionalShots = sk_metrocop_max_additional_ammo_shots.GetInt();
				}
			}
...
float CNPC_MetroPolice::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	...
	case HITGROUP_STOMACH:
	{
		if (info.GetDamageType() & DMG_BUCKSHOT)
		{
			...
			if (flDist > 252.0f)
			{
				return sk_metrocop_vest.GetFloat();
			}
		}
...
WeaponProficiency_t CNPC_MetroPolice::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon)
{
	if (FClassnameIs(pWeapon, "weapon_pistol") || FClassnameIs(pWeapon, "weapon_357"))
	{
		...
		{
			return WEAPON_PROFICIENCY_POOR;	// 20 Poor accuracy for this map, looks silly when gordon can take so many bullets before dying!
		}
...
"		TASK_ANNOUNCE_ATTACK			3"	// 3 = harass
...
"		TASK_GET_FLANK_RADIUS_PATH_TO_ENEMY_LOS	240"
"		TASK_SPEAK_SENTENCE				6"	// METROPOLICE_SENTENCE_MOVE_INTO_POSITION
"		TASK_RUN_PATH					0"
"		TASK_METROPOLICE_RESET_LEDGE_CHECK_TIME 0"
"		TASK_WAIT_FOR_MOVEMENT			0"
...
"		TASK_FACE_ENEMY						0"
"		TASK_SPEAK_SENTENCE					1"	// METROPOLICE_SENTENCE_HES_OVER_HERE
"		TASK_WAIT_FACE_ENEMY				3" // give the guy some time to come out on his own
...
DEFINE_SCHEDULE
(
SCHED_METROPOLICE_MOVE_TO_MELEE,

"	Tasks"
"		TASK_STOP_MOVING				0"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_METROPOLICE_ESTABLISH_LINE_OF_FIRE"
"		TASK_SPEAK_SENTENCE					2"	// METROPOLICE_SENTENCE_HES_RUNNING
"		TASK_GET_CHASE_PATH_TO_ENEMY	300"
"		TASK_RUN_PATH					0"
"		TASK_WAIT_FOR_MOVEMENT	0"
"		TASK_FACE_ENEMY			0"
