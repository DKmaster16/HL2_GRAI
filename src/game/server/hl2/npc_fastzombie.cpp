...
//-----------------------------------------------------------------------------
/*
bool CFastZombie::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
...
//		m_hBlockingProp = NULL;
//		return SCHED_FASTZOMBIE_SMASH_PROP;
//	}
	return baseResult;
}
...
int CFastZombie::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	else if (flDist < FASTZOMBIE_MINLEAP)
	{
		return COND_NONE;
	}
...
void CFastZombie::LeapAttackTouch( CBaseEntity *pOther )
{
	...
	ClawAttack(GetClawAttackRange(), sk_zombie_fast_dmg_both_slash.GetFloat(), qaPunch, forward, ZOMBIE_BLOOD_BOTH_HANDS);
