...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_Manhack::GetAutoAimRadius()
{
...
bool CNPC_Manhack::OverrideMove( float flInterval )
{
	...
	// So cops, etc. will try to avoid them. SOUND_DANGER replaced with SOUND_MOVE_AWAY
	if ( !HasSpawnFlags( SF_MANHACK_NO_DANGER_SOUNDS ) && !m_bHeld )
	{
		CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin(), 75, flInterval, this);
	}
...
void CNPC_Manhack::StartEye( void )
{
	...
	m_hLightTrail = CSpriteTrail::SpriteTrailCreate("sprites/bluelaser1.vmt", GetLocalOrigin(), false);

	if (m_hLightTrail != NULL)
	{
		m_hLightTrail->FollowEntity(this);
		m_hLightTrail->SetAttachment(this, LookupAttachment("Light"));
		if (m_bHackedByAlyx)
		{
			m_pLightGlow->SetTransparency(kRenderTransAdd, 0, 255, 0, 128, kRenderFxNoDissipation);
			m_pLightGlow->SetColor(0, 255, 0);
		...
		m_hLightTrail->SetStartWidth(8.0f);
		m_hLightTrail->SetEndWidth(1.0f);
		m_hLightTrail->SetLifeTime(0.5f);
	}
	*/
}
