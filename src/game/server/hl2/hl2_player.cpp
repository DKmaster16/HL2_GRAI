...
#define PLAYER_HULL_REDUCTION	0.7
...
static ConVar sv_weapon_drop_enabled("sv_weapon_drop_enabled", "0");
...
#define	FLASH_DRAIN_TIME	 1.6667	// 100 units / 90 secs
#define	FLASH_CHARGE_TIME	 15.0f	// 100 units / 2 secs . . . NO, 10 seconds!
...
#else
	CSuitPowerDevice SuitDeviceSprint( bits_SUIT_DEVICE_SPRINT, 10.0f );				// 100 units in 8 seconds . . . nope, 10 seconds
#endif

#ifdef HL2_EPISODIC
	CSuitPowerDevice SuitDeviceFlashlight( bits_SUIT_DEVICE_FLASHLIGHT, 1.667 );	// 100 units in 60 second
#else
...
bool CHL2_Player::IsIlluminatedByFlashlight( CBaseEntity *pEntity, float *flReturnDot )
{
	...
	if (!FVisible(pEntity) && hl2_episodic.GetBool())
		return false;
...
void CHL2_Player::GetAutoaimVector( autoaim_params_t &params )
{
	...
	params.m_fScale *= HL2GameRules()->GetAutoAimScale(pPlayer);
