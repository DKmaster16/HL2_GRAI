...
//
// How long between physics swat attacks (in seconds) (was 5). 
...
ConVar zombie_basemin( "zombie_basemin", "85" );
ConVar zombie_basemax( "zombie_basemax", "115" );
	...
	case HITGROUP_HEAD:
		{
			if( info.GetDamageType() & DMG_BUCKSHOT )
			{
				...
				{
					return (sk_zombie_head_buckshot_pointblank.GetFloat());
				}
			}
			else if (info.GetAmmoType() == GetAmmoDef()->Index("357"))
			{
				return (sk_zombie_head_357.GetFloat());
			}
			else
			{
				return (sk_zombie_head.GetFloat());
			}
		}
	}
...
void CNPC_BaseZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	if (ptr->hitgroup == HITGROUP_LEFTLEG || ptr->hitgroup == HITGROUP_RIGHTLEG || ptr->hitgroup == HITGROUP_STOMACH)
	{
		m_bLegShot = true;
	}
...
bool CNPC_BaseZombie::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	if ( (info.GetDamageType() & DMG_BLAST) && flDamageThreshold >= 0.35)
	{
		return true;
	}
...
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab( const CTakeDamageInfo &info, float flDamageThreshold )
{
	if ( m_iHealth <= 0 )
	{
		...
		if ( info.GetDamageType() & DMG_BULLET )
		{
			if( m_bHeadShot ) 
			{
				...
				if ( pPlayer != NULL )
				{
					...
					if ( pHL2GameRules->NPC_ShouldDrainPistolAmmo( pPlayer ) && random->RandomInt(0, 2) > 0 )	// Half the time
				...
				{
					if (random->RandomInt(0, 1) == 0)	// Half the time
						return RELEASE_IMMEDIATE;
					else
						return RELEASE_RAGDOLL_BULLET;
				}
		...
		if ( info.GetDamageType() & DMG_CLUB)
		{
			...
			{
				// Killed by combine soldier, ragdoll the crab
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	{
		info.ScaleDamage( sk_zombie_bullet_damage_scale.GetFloat() );
	}

	if (info.GetDamageType() & DMG_CLUB)
	{
		if (GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE)
		{
			// Hit by combine soldier, ragdoll the crab
			info.ScaleDamage(5.0);
		}
		else
		{
			// hit with a crowbar. Normal damage
...
bool CNPC_BaseZombie::IsChopped( const CTakeDamageInfo &info )
{
	...
	if ( m_iHealth > 0 || flDamageThreshold <= 0.35 )
		return false;
...
void CNPC_BaseZombie::GatherConditions( void )
{
	...
	if( m_NPCState == NPC_STATE_COMBAT && !m_fIsTorso )
	{
		...
		if( gpGlobals->curtime >= m_flNextSwatScan && (m_hPhysicsEnt == NULL) )
		{
			...
			m_flNextSwatScan = gpGlobals->curtime + 1.0;
		}
	}
...
void CNPC_BaseZombie::BecomeTorso( const Vector &vecTorsoForce, const Vector &vecLegsForce )
{
	...
	if ( !m_fIsHeadless )
	{
		...
		{ 
			m_iMaxHealth = ZOMBIE_TORSO_HEALTH_FACTOR * 1.5 * m_iMaxHealth;
		}
